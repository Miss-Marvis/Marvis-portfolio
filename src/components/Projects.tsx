  ...
  return <section id="projects" className="py-20 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">A selection of projects that showcase my skills in frontend development, UI/UX design, and problem-solving.</p>
        </div>

        <div className="grid lg:grid-cols-2 gap-8">
          {projects.map((project, index) => <Card key={index} className={`bg-card-gradient border-border overflow-hidden hover:shadow-glow-accent transition-all duration-300 transform hover:-translate-y-2 ${project.featured ? 'lg:col-span-2' : ''}`}>
              <div className={`${project.featured ? 'md:flex' : ''}`}>
                <div className={`${project.featured ? 'md:w-1/2' : ''}`}>
                  <div className="aspect-video bg-muted bg-cover bg-center" style={{
                ...
                <div className={`${project.featured ? 'md:w-1/2' : ''}`}>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      {project.featured && <Badge variant="default" className="bg-primary text-primary-foreground">
                          Featured
                        </Badge>}
                    </div>
                  </CardHeader>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {project.tech.map((tech, techIndex) => <Badge key={techIndex} variant="secondary" className="text-xs">
                          {tech}
                        </Badge>)}
                    </div>
